...
const ProjectPage = async ({ params }: { params: Promise<{ href: string }> }) => {
  ...
  return (
    <ContainerWrapper>
      ...
      <div className="p-2 md:p-4">
        <Card className={cn("p-2 group")}>
          <div
            className={cn(
              "bg-muted border-border relative md:h-[500px] h-[200px] sm:h-[400px] w-full overflow-hidden rounded-[6px] border select-none",
          ...
          >
            <h1 className="text-muted-foreground absolute top-3 left-2 text-xs sm:text-base font-medium transition-all duration-300 group-hover:left-1/2 group-hover:-translate-x-1/2">
              {project.title}
            </h1>
            <div className="bg-background absolute bottom-0 left-1/2 h-[80%] sm:h-[90%] w-[90%] -translate-x-1/2 rounded-t-[6px] p-[2px] pb-0 transition-all duration-300 group-hover:h-[85%]">
      ...
      {/* Links Section */}
      <div className="flex w-full p-2 md:p-4 gap-x-4">
        {
          links.map((item) => {
            return (
              <a
                ...
                href={item.href}
                target="_blank"
                className="border-edge w-1/2 group relative flex cursor-pointer flex-row rounded-sm border-[1px] border-dashed"
              >
                <div key={item.name} className="flex w-full items-center gap-x-2 p-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-muted-foreground text-sm hidden sm:block">{item.description}</p>
                  </div>
      ...
      <div className="flex flex-col gap-2 p-2 sm:p-4">
        <a href={project.liveLink} className="text-primary/90 text-base font-bold sm:text-xl md:text-2xl">
          {project.title}
        </a>
